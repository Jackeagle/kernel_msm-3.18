...
static char __init *dm_chrome_parse_target(char *str, struct dm_chrome_target *tgt)
{
	...
	while (options && *options) {
		...
		if (!strcmp(key, "alg")) {
			...
			err = kstrtoul(val, 10, &hashstart);
			if (err)
                                return ERR_PTR(err);
			hashstart = hashstart >> (12 - SECTOR_SHIFT);
	...
	length = strlen(data_device) + strlen(hash_device) + strlen(digest) +
		 strlen(algorithm) + strlen(salt) + 48;
	if (error_behavior)
	...
	snprintf(tgt->field[3], length, "0 %s %s 4096 4096 %lu %lu %s %s %s",
