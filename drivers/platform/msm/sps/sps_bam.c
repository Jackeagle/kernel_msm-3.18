				    ...
				    void *user, u32 flags)
{
	...
	if (dev->ipc_loglevel == 0)
		SPS_DBG(dev,
			"sps:BAM %pa pipe %d addr 0x%pK size 0x%x flags 0x%x\n",
			BAM_ID(dev), pipe_index,(void *)(long)addr,
			size, flags);
...
int sps_bam_pipe_inject_zlt(struct sps_bam *dev, u32 pipe_index)
{
	...
	read_p = sps_bam_pipe_get_desc_read_offset(dev, pipe);
	write_p = sps_bam_pipe_get_desc_write_offset(dev, pipe);
