					...
					int enable,	bool dapm)
{
	...
	tavil_cdc_mclk_enable(codec, enable);
#endif
	return 0;
}
...
#if defined(CONFIG_WCD9335) || defined(CONFIG_SND_SOC_WCD934X)
static int msm_audrx_init_wcd(struct snd_soc_pcm_runtime *rtd)
{
	...
	*/
	...
	/* start mbhc */
	#ifdef CONFIG_SND_SOC_WCD_MBHC
	wcd_mbhc_cfg.calibration = def_ext_mbhc_cal();
	if (!strcmp(dev_name(codec_dai->dev), "tavil_codec")) {
...
#ifdef CONFIG_SND_SOC_WCD934X
static struct snd_soc_dai_link msm8x16_934x_dai[] = {
	/* Backend DAI Links */
	{
		.name = LPASS_BE_QUAT_MI2S_RX,
		.stream_name = "Quaternary MI2S Playback",
...
#ifdef CONFIG_SND_SOC_WCD934X
struct snd_soc_card snd_soc_card_934x_msm8916 = {
	.name		= "msm8x09-tavil934x-snd-card",
	.dai_link	= msm8x16_934x_dai_links,
	.num_links	= ARRAY_SIZE(msm8x16_934x_dai_links),
};
#endif
static struct snd_soc_card bear_cards[MAX_SND_CARDS] = {
#ifdef CONFIG_SND_SOC_WCD934X
	{
		.name		= "msm8x16-tavil-snd-card",
		.dai_link	= msm8x16_934x_dai_links,
		.num_links	= ARRAY_SIZE(msm8x16_934x_dai_links),
	},
#endif
					...
					struct platform_device *pdev)
{
	...
	u32 max_aux_dev = 0;
	int ret, i, found = 0, num_links, num_strings;
	u32 *index = NULL;
	pr_err("%s: CARD is 9340\n", __func__);
	card = &bear_cards[pdev->id];
	num_links = ARRAY_SIZE(msm8x16_934x_dai_links);

	memcpy(msm8x16_934x_dai_links, msm8x16_dai,
			sizeof(msm8x16_dai));
...
static int msm8x16_asoc_machine_probe(struct platform_device *pdev)
{
	...
	if (!pdata->vaddr_gpio_mux_pcm_ctl) {
		pr_err("%s ioremap failure for addr %x",__func__,
